...
def _gen_vocab():
    ...
    corpus = get_all_corpus()  #获取所有古诗
    char_cnts = dict()
    for idx, poem in enumerate(corpus):
        for sentence in poem['sentences']: # 古诗词内容
            for ch in sentence:
                char_cnts[ch] = char_cnts[ch]+1 if ch in char_cnts else 1  # count words[字符]
        if 0 == (idx+1)%10000:
            print ("[Vocabulary] %d/%d poems have been processed." %(idx+1, len(corpus)))
    vocab = sorted([ch for ch in char_cnts], key = lambda ch: -char_cnts[ch])[:VOCAB_SIZE-2] # 降序排列
    with codecs.open(_vocab_path, 'w', 'utf-8') as fout:
        json.dump(vocab, fout) #save
...
def get_vocab():
    ...
    int2ch.append(u'<PAD>')
    ch2int = dict((ch, idx) for idx, ch in enumerate(int2ch)) #word-idx dict()
