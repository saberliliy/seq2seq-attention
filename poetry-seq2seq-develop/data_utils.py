...
def prepare_batch_predict_data(keyword, previous=[], prev=True, rev=False, align=False):
    ...
    for sentence in previous:                    # if previous=[] 整个for 循环都不执行
        ...
        while not stop:
            ...
            for i in range(batch_size):
                ...
                if not line:
                    ...
                    current_sentence_ints = process_sentence(current_sentence, rev=rev, pad_len=7 if align else None)   #将句子转换成数字，并且进行填充
